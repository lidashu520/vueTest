    ...
    <ul class="formCom">
      ...
      <li class="icon-arrow">
        ...
        <select v-model='data.selectSex'>
          <option v-for='option in optionSex' :value='option.txt'>{{option.txt}}</option>
        </select>
      </li>
      <li class="icon-arrow">
        <span>学历</span>
        ...
        <select v-model='data.selectEdu'>
          <option v-for='option in optionEdu' :value='option.txt'>{{option.txt}}</option>
        </select>
      </li>
      <li class="icon-arrow">
        <span>婚姻状况</span>
        ...
        <select v-model='data.selectMar'>
          <option v-for='option in optionMar' :value='option.txt'>{{option.txt}}</option>
        </select>
      </li>
      ...
      <li class="icon-arrow">
       ...
       <li class="icon-arrow">
        ...
        <select v-model='data.selectPay'>
          <option v-for='option in optionPay' :value='option.txt'>{{option.txt}}</option>
        </select>
      </li>
      <li>
        <label>
    ...
    <p class="listTop questionsWord ">二、营养状况调查</p>
     ...
     <dt style="padding-top: 0.325rem"><span>1.  您认为你目前健康状况如何？  </span></dt>
      <el-radio-group v-model="radio1">
        <dd>
          <el-radio :label="1"><span class="questionsWord">A．非常好 </span></el-radio>
          <el-radio :label="2"><span class="questionsWord">B. 好 </span></el-radio>
          <el-radio :label="3"><span class="questionsWord">C．一般 </span></el-radio>
          <el-radio :label="4"><span class="questionsWord">D. 较差 </span></el-radio>
          <el-radio :label="5"><span class="questionsWord">E. 非常差 </span></el-radio>
        </dd>
      </el-radio-group>
     </dl>
     ...
     <dt style="padding-top: 0.325rem"><span>5. 您昨天具体吃了些什么？（具体食物和数量)  </span></dt>
      <el-radio-group v-model="radio5" @change="showSix">
        <dd>
          <el-radio  :label="1"><span class="questionsWord">A．是（继续回答6题） </span></el-radio>
          <el-radio  :label="2"><span class="questionsWord">B. 否（结束测评） </span></el-radio>
        </dd>
      </el-radio-group>
    ...
    <dl v-show="isShow">
     <dt  style="padding-top: 0.325rem"><span>6. 您多久服用一次营养补充剂呢？  </span></dt>
      <el-radio-group v-model="radio6">
        <dd>
          <el-radio :label="1"><span class="questionsWord">A.2/日以上 </span></el-radio>
          <el-radio :label="2"><span class="questionsWord">B. 2/日 </span></el-radio>
          <el-radio :label="3"><span class="questionsWord">C. 1/日 </span></el-radio>
          <el-radio :label="4"><span class="questionsWord">D. 2-3/周 </span></el-radio>
          <el-radio :label="5"><span class="questionsWord">E．1/周 </span></el-radio>
        </dd>

        <dd>
          <el-radio :label="6"><span class="questionsWord">F. 其他 </span></el-radio>
          <input class="questionsWord" type="text" placeholder="具体多久一次" v-model.trim='data.dinner'>
        </dd>
      </el-radio-group>
  ...
  export default {
    ...
    methods: {
      saveInfo() {
        ...
        else if (checkTel.test(this.data.tel) === false) this.$dialog("电话格式错误");
        ...
        else if (this.radio1 === 0) this.$dialog("请作答1题");
        else if (this.radio2 === 0) this.$dialog("请作答2题");
        else if (this.checkList.length ===0) this.$dialog("请作答3题的");
        else if (this.radio3 === 0) this.$dialog("请作答3题");
        else if (this.radio4 === 0) this.$dialog("请作答4题");
        else if (this.radio5 === 0) this.$dialog("请作答5题");
        else if (this.radio5 === 1) {
           if (this.radio6 === 0) this.$dialog("请作答6题");
        }
        else {
      ...
      showSix() {
        if(this.radio5 === 2) this.isShow = false
        if(this.radio5 === 1) this.isShow = true
    },
    },
    filters: {
